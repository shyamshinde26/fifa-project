...
plt.show()
value_distribution_values=df['Wage (M)'].sort_values(ascending=False).head(100)
value_distribution_values.value_counts().plot.bar()
plt.show()
overall=df['Wage (M)'].sort_values()
overall_value=df['Value (M)'].groupby(overall).mean()
overall.value_counts().plot.bar()
plt.show()
df['Value (M)'].value_counts().plot.bar()
plt.show()
# Code ends here
...
mae=mean_absolute_error(y_pred_2,y_test)
